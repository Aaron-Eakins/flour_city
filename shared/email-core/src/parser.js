// parser.js — Received-chain parser core (pure ESM, no I/O, no environment deps).
// Header text in, structured data out. Canonical home for the parser shared by the
// email-parser CLI, the labs browser analyzer, and the Cloudflare email worker.

// --- Stage 1: unfold ---
// RFC 5322 header "folding": a logical header line can be split across multiple
// physical lines, where every continuation line begins with a space or tab. To
// parse reliably we first stitch those back together.
...
// Kept in order because order is everything for the Received chain. The blank
// line separating headers from body ends parsing.
export function splitHeaders(logicalLines) {
  const headers = [];
...
export function parseReceivedValue(value) {
  const hop = {
    from: null,
    fromIp: null,
    by: null,
  ...
  // Strip parenthetical comments before clause extraction. Servers stuff TLS
  // details into parens like "(using TLSv1.3 with cipher ...)", and the word
  // "with" inside there would otherwise be mistaken for the protocol clause.
  // (We already saved the IP above.)
...
// Received headers are prepended by each server, so the topmost Received is the
// LAST hop (closest to the inbox) and the bottom one is the FIRST. We return them
// in chronological order (oldest first) which is how a human reads the journey,
// and tag each with its position.
...
// Convenience: full header list from raw text (used by the worker path).
