...
// @ts-ignore: Deno is built-in to the Supabase runtime
const CF_INBOUND_SECRET = Deno.env.get('CF_INBOUND_SECRET')
...
Deno.serve(async (req: Request) => {
  ...
  if (CF_INBOUND_SECRET && authHeader !== `Bearer ${CF_INBOUND_SECRET}`) {
    console.error('Unauthorized request attempt from Cloudflare Bridge.')
