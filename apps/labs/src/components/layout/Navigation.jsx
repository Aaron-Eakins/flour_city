...
const Navigation = ({ view, setView, isScrolled, isHome, openAuth }) => {
    ...
    const isServicesActive = view === 'email' || view === 'printing' || view === 'services' || view === 'web-design';
                ...
                <div className="hidden md:flex items-center space-x-8 text-[11px] font-black tracking-widest uppercase text-[#F2F1EF]">
                    ...
                    <div className="relative" onMouseEnter={() => setServicesDropdownOpen(true)} onMouseLeave={() => setServicesDropdownOpen(false)}>
                        <button
                            ...
                            className={`flex items-center gap-1 hover:text-[#D4A017] transition-colors ${isServicesActive ? 'text-[#D4A017]' : ''}`}
                        >
                            Services <ChevronDown size={11} className={`transition-transform duration-200 ${servicesDropdownOpen ? 'rotate-180' : ''}`} />
                        </button>
                        {servicesDropdownOpen && (
                            <div className="absolute top-full left-0 pt-3">
                                <div className="bg-[#1A1B1E] border border-white/10 shadow-2xl py-2 min-w-[200px]">
                                    <button onClick={() => handleLinkClick('email')} className={`w-full text-left px-5 py-3 text-[11px] font-black tracking-widest uppercase hover:text-[#D4A017] hover:bg-white/5 transition-colors ${view === 'email' ? 'text-[#D4A017]' : ''}`}>
                ...
                <div className="md:hidden absolute top-full left-0 w-full bg-[#1A1B1E] border-t border-white/5 animate-in slide-in-from-top duration-300 overflow-hidden">
                    <div className="flex flex-col p-6 space-y-1 text-[12px] font-black tracking-[0.3em] uppercase text-[#F2F1EF]">
                        ...
                        <div className="border-b border-white/5">
                            <button onClick={() => setServicesMobileOpen(!servicesMobileOpen)} className={`w-full text-left py-3 flex items-center justify-between ${isServicesActive ? 'text-[#D4A017]' : 'text-gray-400'}`}>
                                <span>Services</span>
                                <ChevronDown size={12} className={`transition-transform duration-200 ${servicesMobileOpen ? 'rotate-180' : ''}`} />
                            </button>
                            {servicesMobileOpen && (
