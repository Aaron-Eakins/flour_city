// loader.js — Input layer (sits in FRONT of @flour-city/email-core).
// File path in -> raw header text out. The parser stays pure; this handles the
// messy job of getting header text out of different file formats. App-specific
// (Node-only: uses fs), so it lives here, not in the shared package.
...
// We only need the part before the first blank line.
export function headersFromEml(text) {
...
export function headersFromMsg(buffer) {
...
export function loadHeaders(filePath) {
